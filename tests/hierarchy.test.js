...
test('parseStructureFields detects milestones and hierarchy metadata', () => {
  ...
  assert.equal(ms.parentId, 'PD-E-9');
  ...
  assert.equal(ch.parentId, 'PD-E-9');
});

test('buildTaskHierarchy builds parent-child tree without altering task ids', () => {
  const tasks = [
    { id: 'PD-E-9', title: 'Phase', parentId: null, release: null, workstream: null, nodeKind: 'phase', isMilestone: false, status: 'To Do' },
    { id: 'PD-E-10', title: 'Child', parentId: 'PD-E-9', release: 'r1', workstream: null, nodeKind: 'task', isMilestone: false, status: 'To Do' },
    { id: 'PD-E-11', title: 'Milestone', parentId: 'PD-E-9', release: 'r1', workstream: null, nodeKind: 'milestone', isMilestone: true, status: 'To Do' },
  ];
  const schedule = new Map([
    ['PD-E-10', { startDate: '2026-08-05', endDate: '2026-08-06' }],
    ['PD-E-11', { startDate: '2026-08-06', endDate: '2026-08-06' }],
  ]);
  const tree = buildTaskHierarchy(tasks, schedule);
  assert.deepEqual(tree.roots, ['PD-E-9']);
  assert.deepEqual(tree.nodes['PD-E-9'].childrenIds, ['PD-E-10', 'PD-E-11']);
  assert.equal(tree.nodes['PD-E-11'].isMilestone, true);
  assert.equal(tree.nodes['PD-E-10'].startDate, '2026-08-05');
});
...
test('normalizeTaskPatch accepts parent_id release and workstream', () => {
  ...
  assert.equal(patch.parent_id, 'PD-E-9');
...
test('gantt API exposes milestones and hierarchy with zero-duration milestone', () => {
  ...
  assert.equal(plan.milestones[0].id, 'PD-E-11');
  assert.equal(plan.milestones[0].durationIaHours, 0);
  assert.equal(plan.milestones[0].startDate, plan.milestones[0].endDate);
  assert.deepEqual(plan.hierarchy.roots, ['PD-E-9']);
  assert.deepEqual(plan.hierarchy.nodes['PD-E-9'].childrenIds, ['PD-E-10', 'PD-E-11']);
