...
test('parseCrossProjectDependency recognizes slug:task tokens', () => {
  const slugs = new Set(['project-demo', 'ariadne']);
  const row = parseCrossProjectDependency('project-demo:PD-E-10:FS+1d', 'ariadne', slugs);
  assert.deepEqual(row, {
    fromProjectSlug: 'project-demo',
    fromTaskId: 'PD-E-10',
    relation: 'FS',
    lagToken: '+1d',
    raw: 'project-demo:PD-E-10:FS+1d',
  });
  assert.equal(parseCrossProjectDependency('PD-E-1', 'ariadne', slugs), null);
});
...
test('collectCrossProjectDependencies indexes edges from task frontmatter', () => {
  ...
  const edges = collectCrossProjectDependencies(tasks, 'ariadne', new Set(['project-demo']));
  assert.equal(edges.length, 1);
  assert.equal(edges[0].fromProjectSlug, 'project-demo');
