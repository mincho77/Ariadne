...
- Contrato completo UI↔Hub: `docs/gantt-ui-integration.md`; la UI externa es opcional y configurable.
...
3. Se abre una ventana en `:63447/?project=project-demo` (ejemplo)
4. La UI pide el plan a Ariadne y lo renderiza

### 3.2 Desde la API (sin UI)

```bash
curl -s "http://127.0.0.1:4177/api/projects/project-demo/gantt?capacity=2&includeDone=0&startDate=2026-08-04"
```

### 3.3 Editar dependencias de una tarea

```bash
curl -s -X POST "http://127.0.0.1:4177/api/projects/project-demo/tasks/dependencies" \
  -H "content-type: application/json" \
  -d '{
    "id": "PD-E-2",
    "dependencies": [
      { "id": "PD-E-1", "relation": "FS" },
      { "id": "PD-E-10", "relation": "SS", "lagValue": 1, "lagUnit": "d" }
    ]
  }'
```
...
| Token | Significado |
|-------|-------------|
| `PD-E-10` | Legacy: equivale a `PD-E-10:FS` sin lag |
| `PD-E-10:FS` | **Finish-to-Start:** B empieza cuando A termina |
| `PD-E-10:SS+1d` | **Start-to-Start:** B empieza 1 día después de que A arranca |
| `PD-E-10:FF+8h` | **Finish-to-Finish:** B termina 8h IA después de que A termina |
| `PD-E-10:SF-1d` | **Start-to-Finish:** B termina 1 día antes/después del inicio de A |
...
```json
{
  ...
  "dependencies": [
    "PD-E-1",
    { "id": "PD-E-3", "relation": "FF", "lagValue": 2, "lagUnit": "d" }
  ]
}
```
...
```json
{
  "project": { "slug": "project-demo", "name": "Proyecto Demo" },
  "parameters": {
    "capacity": 2,
    "includeDone": false,
    "iaHoursPerDay": 8,
  ...
  "criticalPath": {
    "estimatedIaHours": 40,
    "route": [
      { "id": "PD-E-1", "title": "...", "durationIaHours": 16 },
      { "id": "PD-E-2", "title": "...", "durationIaHours": 24 }
    ]
  },
  "parallelGroups": [
    { "startIaHour": 0, "startDate": "2026-08-04", "ids": ["PD-E-5", "PD-E-6"] }
  ],
  "dependencyEdges": [
    {
      "fromId": "PD-E-1",
...
# Plan
curl -s "http://127.0.0.1:4177/api/projects/project-demo/gantt?includeDone=0" \
  | python3 -m json.tool | head -80

# Capacidad IA
curl -s http://127.0.0.1:4177/api/projects/project-demo/ai-capacity-config
```
