...
| Campo | Uso |
|-------|-----|
| `slug` | Identificador en URLs (`?project=project-demo`) |
...
- `projectTaskCode()` — resuelve código desde `taskCode`, defaults (`ariadne→AH`, `project-demo→JM`) o iniciales del slug
...
(o ejecuta `npm run queue:complete -- project-demo PD-B-23`).

Proyecto: Proyecto Demo (project-demo)
Ruta: /path/to/project-demo
Backlog: backlog/tasks/pd-b-23 - ...
```
...
```bash
npm run queue:complete -- project-demo PD-B-23
```
...
El ledger **no reemplaza** las tareas de backlog; las referencia por ID (`PD-B-1`, `AH-E-3`).
...
npm run task:create -- project-demo --bug "BUG · Upload congela" --priority "Ultra High"

# Mejora sin cola
npm run task:create -- ariadne --mejora "HUB · Auditoría" --priority High

# Normalización dry-run
npm run task:normalize -- project-demo
npm run task:normalize -- project-demo --apply
...
| Plist | Label | Proceso |
|-------|-------|---------|
| `com.ariadne.hub.plist` (generado) | `com.ariadne.hub` | `node server.js` — **único Hub** |
| `com.example.ariadne-bug-queue.plist` | `com.example.ariadne-bug-queue` | runner bugs (opcional) |
| ~~`com.example.ariadne-hub-legacy`~~ | retirado | conflicto de puertos — bootout obligatorio |

Guía: [`docs/launchagent-hub.md`](launchagent-hub.md).

**Instalación Hub (recomendado):**

```bash
npm run hub:launchagent
```

**Bootout del Hub legado:**

```bash
launchctl bootout gui/$(id -u)/com.example.ariadne-hub-legacy 2>/dev/null || true
rm -f ~/Library/LaunchAgents/com.example.ariadne-hub-legacy.plist
```

**Cola de bugs (opcional):**

```bash
cp com.example.ariadne-bug-queue.plist ~/Library/LaunchAgents/
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.example.ariadne-bug-queue.plist
launchctl kickstart -k gui/$(id -u)/com.example.ariadne-bug-queue
```

**Logs:**

- `/tmp/ariadne-hub.log`
- `/tmp/ariadne-bug-queue-demo.log`
...
Con el Hub activo, Gantt vive en el mismo proceso (`/api/gantt/portfolio`, `/portfolio.html`).
...
└── com.example.ariadne-bug-queue.plist  # LaunchAgent runner
```
...
7. **Reemplazo de Proyecto Demo hub** — Este repo sustituye `Proyecto Demo/tools/ariadne-hub`. El LaunchAgent viejo `com.example.ariadne-hub-legacy` debe permanecer apagado para evitar conflicto en puerto 4177.
...
curl -X POST 'http://127.0.0.1:6421/api/bugs/create?project=project-demo' \
  -H 'content-type: application/json' \
  -d '{"title":"BUG · ...","priority":"Ultra High"}'

# Consultar cola
curl 'http://127.0.0.1:6421/api/queue/bugs?project=project-demo'
...
1. LaunchAgent `com.example.ariadne-bug-queue` hace poll
...
| Kanban Proyecto Demo bugs | http://127.0.0.1:6421/?project=project-demo&view=bugs |
