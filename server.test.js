...
test('slugify produces stable local ids', () => assert.equal(slugify('Proyecto Demo IA / Tokens'), 'proyecto-demo-ia-tokens'));
...
test('parseTask unfolds YAML multiline titles', () => { const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'ariadne-task-')); const file = path.join(dir, 'PD-19 - Pretensiones.md'); fs.writeFileSync(file, '---\nid: PD-19\ntitle: >-\n  BUG producción · Justo no resuelve extracción\n  de pretensiones\nstatus: To Do\n---\n'); assert.equal(parseTask(file).title, 'BUG producción · Justo no resuelve extracción de pretensiones'); });
...
test('pickNextImprovement ignores bugs and picks active improvement work', () => {
  const tasks = [
    { id: 'PD-2', title: 'Feature alta', status: 'In Progress', priority: 'Ultra High', type: 'feature' },
    { id: 'PD-1', title: 'BUG upload', status: 'To Do', priority: 'Medium', type: 'bug' },
    { id: 'PD-3', title: 'Otra mejora', status: 'To Do', priority: 'Low', type: 'task' },
  ];
...
test('summarize exposes split bug and improvement lanes for the hub', () => {
  ...
  assert.match(summary.next, /PD-2/);
});
...
test('queue board exposes ordered queue and drag targets for every status', () => {
  ...
  assert.match(html, /assertClientDoneEvidence/);
  assert.match(html, /Falta ## Evidence o ## Closeout/);
});
...
test('validateTaskSource requires frontmatter and stable id', () => {
  assert.throws(() => validateTaskSource('PD-1', 'sin frontmatter'), /frontmatter/);
  assert.throws(() => validateTaskSource('PD-1', '---\nid: PD-2\n---\n'), /no cambies el id/);
  assert.doesNotThrow(() => validateTaskSource('PD-1', '---\nid: PD-1\n---\n\n## Description\n\nTexto'));
});
test('updateTaskSource writes markdown back to backlog file', () => {
  ...
  const updated = updateTaskSource(project, 'PD-1', '---\nid: PD-1\ntitle: Demo editado\nstatus: To Do\nupdated_date: \'2026-07-28 00:00\'\n---\n\n## Description\n\nDespués');
...
test('queue board exposes substatus controls', () => {
  ...
  const dir = path.join(root, 'backlog', 'tasks');
  fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(path.join(dir, 'pd-e-21 - E2E.md'), '---\nid: PD-E-21\ntitle: VALIDACIÓN E2E\nstatus: In Progress\nsubstatus: "Pendiente Resultado Prueba"\nnext_action: "La persona responsable valida TASK-123"\ntype: task\npriority: High\n---\n');
  const html = queueBoardPage({ name: 'Demo', slug: 'demo', path: root });
  assert.match(html, /Pendiente Resultado Prueba/);
  assert.match(html, /La persona responsable valida TASK-123/);
...
test('createTask allocates typed bug and enhancement ids', () => {
  ...
  const project = { slug: 'project-demo', name: 'Proyecto Demo', taskCode: 'PD', path: root };
...
test('buildProjectGantt schedules dependencies before dependents', () => {
  ...
  assert.ok(byId.get('PD-E-1').startDay <= byId.get('PD-E-2').startDay);
  assert.ok(byId.get('PD-E-2').startDay <= byId.get('PD-E-3').startDay);
...
test('buildProjectGantt enforces FS/SS/FF/SF constraints with lag', () => {
  ...
  const byId = new Map(plan.tasks.map((task) => [task.id, task]));
  const base = byId.get('PD-E-1');
  const ss = byId.get('PD-E-2');
  const fsTask = byId.get('PD-E-3');
  const ff = byId.get('PD-E-4');
  const sf = byId.get('PD-E-5');
  ...
  const ffEdge = plan.dependencyEdges.find((edge) => edge.fromId === 'PD-E-1' && edge.toId === 'PD-E-4');
  const sfEdge = plan.dependencyEdges.find((edge) => edge.fromId === 'PD-E-1' && edge.toId === 'PD-E-5');
...
test('updateTaskDependencies persists typed relation tokens in frontmatter', () => {
  ...
  const updated = updateTaskDependencies(project, 'PD-E-2', [
    { id: 'PD-E-1', relation: 'FF', lagValue: 1, lagUnit: 'd' },
  ]);

  assert.deepEqual(updated.dependencies, ['PD-E-1']);
  ...
  assert.match(source, /dependencies:[\s\S]*PD-E-1:FF\+1d/);
});
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/projects/demo-http/tasks/dependencies`, {
      ...
      body: JSON.stringify({
        id: 'PD-E-2',
        dependencies: [
          { id: 'PD-E-1', relation: 'SS', lagValue: 2, lagUnit: 'h' },
        ],
      }),
    });
    assert.equal(response.status, 200);
    const payload = await response.json();
    assert.equal(payload.id, 'PD-E-2');
    ...
    assert.match(source, /dependencies:[\s\S]*PD-E-1:SS\+2h/);
  } finally {
...
test('dependencies endpoint rejects missing id over HTTP', { timeout: 20000 }, async () => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/projects/demo-http/tasks/dependencies`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ dependencies: [{ id: 'PD-E-1', relation: 'FS' }] }),
    });
...
test('dependencies endpoint rejects self dependency over HTTP', { timeout: 20000 }, async () => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/projects/demo-http/tasks/dependencies`, {
      ...
      body: JSON.stringify({
        id: 'PD-E-2',
        dependencies: [{ id: 'PD-E-2', relation: 'FS' }],
      }),
    });
    assert.equal(response.status, 400);
    const payload = await response.json();
    ...
    assert.doesNotMatch(source, /PD-E-2:FS/);
  } finally {
    await stopProcess(child);
  }
});
...
test('dependencies endpoint rejects invalid relation and malformed lag over HTTP', { timeout: 20000 }, async () => {
  ...
  try {
    const invalidRelation = await fetch(`http://127.0.0.1:${port}/api/projects/demo-http/tasks/dependencies`, {
      ...
      body: JSON.stringify({
        id: 'PD-E-2',
        dependencies: [{ id: 'PD-E-1', relation: 'XS' }],
      }),
    });
...
test('dependencies endpoint returns 404 for unknown project over HTTP', { timeout: 20000 }, async () => {
  ...
  try {
    const response = await fetch(`http://127.0.0.1:${port}/api/projects/no-existe/tasks/dependencies`, {
      ...
      body: JSON.stringify({
        id: 'PD-E-2',
        dependencies: [{ id: 'PD-E-1', relation: 'FS' }],
      }),
    });
