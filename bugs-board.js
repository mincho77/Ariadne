...
function renderBugStatsHtml(stats, escapeHtml) {
  ...
  return `
    <section class="stats-grid">
      <article class="kpi"><span>Total bugs</span><strong>${stats.total}</strong></article>
      ...
      <article class="kpi"><span>Tasa de cierre</span><strong>${stats.closeRate}%</strong></article>
    </section>
    <section class="analytics">
      ...
      <article class="panel-card">
        ...
        <div class="table-wrap">
          <table>
            <thead><tr><th>Tema</th><th>Total</th><th>Por hacer</th><th>En curso</th><th>Cola</th><th>Resueltos</th><th>Cierre</th></tr></thead>
...
function bugsBoardPage(project, helpers) {
  ...
  const columns = [
    ...
    { status: 'Queued', label: 'Cola', hint: 'Autorizados', queue: true },
  ...
  const cards = columns.map(({ status, label, hint, queue }) => {
    ...
    const content = items.map((task, position) => {
      ...
      const queuePosition = queue ? `<span class="queue-position"><small>Turno</small>${position + 1}</span>` : '';
      return `<button class="task type-bug-card${queue ? ' queue-task' : ''}" draggable="true" data-task="${index}" data-task-id="${escapeHtml(task.id)}" data-status="${escapeHtml(task.status)}" data-search="${escapeHtml(searchable)}">
        ${queuePosition}
        ...
        <span class="drag-hint">⋮⋮ Arrastrar para mover estado</span>
      </button>`;
    }).join('') || '<p class="empty">Sin bugs en esta columna</p>';
    return `<section class="column${queue ? ' queue-column' : ''}" data-column="${escapeHtml(status)}">
      <header class="column-head"><div><span class="column-kicker">${escapeHtml(hint)}</span><h2>${escapeHtml(label)}</h2></div><span class="column-count">${items.length}</span></header>
  ...
  const boardUrl = `http://${HOST}:${BOARD_PORT}/?project=${encodeURIComponent(project.slug)}`;
...
<style>
:root{color-scheme:dark;font:15px Inter,ui-sans-serif,system-ui,sans-serif;background:#120b0d;color:#f8eef0}
*{box-sizing:border-box}body{margin:0;padding:28px;max-width:1680px;margin:auto;background:radial-gradient(circle at 20% -10%,#3b1820 0,transparent 32%),#120b0d}
a{color:#ff9eb3}.eyebrow{color:#ff7d98;letter-spacing:.14em;font-size:11px;font-weight:800}h1{margin:6px 0 4px;font-size:32px}.muted{color:#c4a9b0}
...
.nav-link{border:1px solid #6c3a48;background:#2a151c;color:#ffc2cf;border-radius:12px;padding:12px 15px;font-weight:800;text-decoration:none}
.stats-grid{display:grid;grid-template-columns:repeat(4,minmax(140px,1fr));gap:12px;margin:18px 0}
...
.analytics{display:grid;grid-template-columns:1fr 1.2fr;gap:14px;margin-bottom:18px}.panel-card{background:#1a1114;border:1px solid #4f2f39;border-radius:15px;padding:16px}.panel-card.compact{grid-column:1/-1}.panel-card h2{margin:0 0 8px;font-size:17px}.hint{color:#b697a0;font-size:12px;margin:0 0 12px}
...
.queue-column{background:linear-gradient(180deg,#301722,#1a1114)}.queue-position{display:inline-grid;place-items:center;width:28px;height:28px;border-radius:8px;background:#ff5d7d;color:#fff;font-size:12px;font-weight:900;margin-bottom:6px}.queue-task{padding-top:4px}
...
<body>
<p class="eyebrow">ARIADNE · MÓDULO DE BUGS</p>
<h1>${escapeHtml(project.name)}</h1>
<p class="muted">${bugs.length} bugs detectados · vista filtrada por <code>type: bug</code>, etiqueta <code>bug</code> o título BUG/SEGURIDAD · <a href="${boardUrl}">Volver al tablero general</a> · <a href="http://${HOST}:${PORT}">Hub</a></p>
${statsHtml}
<div class="toolbar">
  ...
  <a class="nav-link" href="${boardUrl}">Tablero completo</a>
</div>
