...
if (fs.existsSync(projectsExamplePath)) {
  ...
  if (!Array.isArray(example) || example.length === 0) {
    ...
    for (const item of example) {
      ...
      if (!projectPath.includes('/path/to/') && !projectPath.includes('/home/user/')) {
...
try {
  ...
  for (const rel of files) {
    if (rel === 'projects.example.json' || rel === 'scripts/check-standalone-ready.js' || rel.endsWith('.md')) continue;
    const full = path.join(root, rel);
    let text;
    try {
      text = readText(full);
    ...
    if (text.includes(localHomePrefix)) {
