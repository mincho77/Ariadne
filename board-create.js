...
function boardCreateInitScript({ type, priority, labels, promptText, buttonLabel, defaultTemplate }) {
  const labelsJson = JSON.stringify(labels || []);
  const templateDefault = defaultTemplate || (type === 'bug' ? 'bug' : 'enhancement');
  return `
...
if(createButton){
  createButton.onclick=async()=>{
    ...
    if(!title)return;
    const templateRaw=prompt('Plantilla AC: bug | enhancement | feature | swarm | minimal | none',${JSON.stringify(templateDefault)});
    if(templateRaw===null)return;
    const template=String(templateRaw||${JSON.stringify(templateDefault)}).trim().toLowerCase()||${JSON.stringify(templateDefault)};
    createButton.disabled=true;
    createButton.textContent='Creating…';
    try{
      const response=await fetch('/api/tasks/create?project='+encodeURIComponent(project),{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({title,type:${JSON.stringify(type)},priority:${JSON.stringify(priority)},labels:${labelsJson},template})});
