...
const { parseStructureFields } = require('./hierarchy');
const { parseResourceFields } = require('./resources');
...
function parseDateStamp(value) {
  // Parse ISO date string while preserving local timezone interpretation.
  // IMPORTANT: Date-only values like '2026-08-11' must be formatted as '2026-08-11T00:00:00'
  // to avoid UTC interpretation that causes 1-day backward shift in non-UTC timezones.
  // See: lib/gantt/restrictions.js::dateToStartIaHour for calendar working-day logic.
...
function toPlanningTask(task, iaHoursPerDay) {
  ...
  const structure = parseStructureFields(task);
  const resourceMeta = parseResourceFields(task);
  const baselineEstimateIaHours = structure.isMilestone ? 0 : estimateTaskIaHours(task, iaHoursPerDay);
  const progressMeta = structure.isMilestone
    ? {
      baselineEstimateIaHours: 0,
      remainingIaHours: 0,
      executedIaHours: 0,
      progress: isDoneStatus(task.status) ? 100 : null,
      progressDeclared: null,
      progressSuggestedFromChecklist: null,
      remainingDeclared: null,
      durationSource: 'milestone',
    }
    : buildProgressSnapshot(task, iaHoursPerDay, baselineEstimateIaHours);
  const durationIaHours = structure.isMilestone
    ? 0
    : resolveEffectiveDurationIaHours(task, iaHoursPerDay, baselineEstimateIaHours);
  return {
    ...task,
    ...
    ...blockMeta,
    ...structure,
    assignees: resourceMeta.assignees,
    requiredSkills: resourceMeta.requiredSkills,
