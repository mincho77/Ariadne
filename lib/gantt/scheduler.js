...
const { buildTaskHierarchy, collectMilestones } = require('./hierarchy');
const {
  defaultResourceConfig,
  resolveTaskPool,
  canAcceptTaskWithResources,
} = require('./resources');
const { buildSlackAnalysis } = require('./slack');
const {
  parseCapacityPolicy,
  taskScheduleLane,
...
function buildProjectGanttFromTasks(tasks, projectMeta, options = {}) {
  ...
  const resourceAware = options.resourceAware === true;
  const resourceConfig = options.resourceConfig || defaultResourceConfig(capacity);
  ...
  const canScheduleTask = (running, task) => (
    resourceAware
      ? canAcceptTaskWithResources(running, task, capacityPolicy, resourceConfig)
      : canAcceptTask(running, taskScheduleLane(task), capacityPolicy)
  );
  ...
  const planTask = (task, startIaHour, scheduleMeta = {}) => {
    ...
    const endCalendar = task.isMilestone
      ? startCalendar
      : calendar.toCalendarMarker(Math.max(0, endIaHour - 1));
    ...
    const item = {
      ...
      resourceType: task.resourceType || 'human',
      resourcePoolId: resourceAware ? resolveTaskPool(task, resourceConfig).id : null,
  ...
  const trackScheduledTask = (task, planned) => {
    if (task.isMilestone) {
      completed.add(task.id);
      return;
    }
    const entry = { id: task.id, endIaHour: planned.endIaHour, lane: planned.lane };
    if (resourceAware) {
      const pool = resolveTaskPool(task, resourceConfig);
      entry.poolId = pool.id;
      entry.primaryAssignee = (task.assignees || [])[0] || null;
    }
    running.push(entry);
  };
  ...
  const seedOperationalTask = (task, startIaHour, scheduleMeta = {}) => {
    ...
    trackScheduledTask(task, planned);
  };
  ...
  for (const task of pendingTasks.filter((item) => isInProgressStatus(item.status)).sort(prioritySort)) {
    ...
    if (!canScheduleTask(running, task)) continue;
  ...
  while (completed.size < pendingTasks.length) {
    ...
    while (canStartMore) {
      ...
      for (const task of unscheduled) {
        ...
        trackScheduledTask(task, plannedTask);
  ...
  const doneTimeline = includeDone
    ? doneTasks.map((task) => ({
      ...
      title: task.title,
      status: task.status,
      durationDays: task.isMilestone ? 0 : task.durationDays,
      durationIaHours: task.isMilestone ? 0 : task.durationIaHours,
      startDay: 0,
      endDay: task.isMilestone ? 0 : task.durationDays,
      startIaHour: 0,
      endIaHour: task.isMilestone ? 0 : task.durationIaHours,
  ...
  const hierarchy = buildTaskHierarchy(planningTasks, schedule);
  const milestones = collectMilestones(planned);
  const slackAnalysis = buildSlackAnalysis(pendingTasks, schedule, dependents, dependencyEdges);
  const slackById = new Map(slackAnalysis.tasks.map((row) => [row.id, row]));
  for (const task of planned) {
    const row = slackById.get(task.id);
    if (!row) continue;
    task.totalSlackIaHours = row.totalSlackIaHours;
    task.freeSlackIaHours = row.freeSlackIaHours;
    task.isCriticalSlack = row.isCritical;
    task.earlyStartIaHour = row.earlyStartIaHour;
    task.lateStartIaHour = row.lateStartIaHour;
  }
  ...
  return {
    ...
    parameters: {
      ...
      resourceAware,
      resourceConfig: resourceAware ? resourceConfig : undefined,
    },
    summary: {
      totalTasks: total,
      doneTasks: done,
      ...
      blockedWithoutUnblockDate: pendingTasks.filter((task) => task.isBlocked && !task.expectedUnblockIso).length,
      lowConfidenceForecasts: planned.filter((item) => item.forecastConfidence === 'low').length,
      milestoneCount: milestones.length,
      hierarchyRoots: hierarchy.roots.length,
      criticalSlackTasks: planned.filter((item) => item.isCriticalSlack).length,
    },
    criticalPath: critical,
    slack: {
      logicalCriticalPath: slackAnalysis.logicalCriticalPath,
      resourceCriticalPath: slackAnalysis.resourceCriticalPath,
      projectEndIaHour: slackAnalysis.projectEndIaHour,
    },
    parallelGroups,
    dependencyEdges,
    monthMarkers,
