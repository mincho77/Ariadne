...
const FRONTEND_REPO = String(
  process.env.ARIADNE_GANTT_UI_REPO || '',
).trim();
...
function buildHubGanttUiConfig({ hubApiBase, ganttBaseUrl, hubPort, boardPort } = {}) {
  ...
  return {
    ...
    endpoints: {
      ...
      taskQueueAuthorize: {
        method: 'POST',
        path: '/api/projects/{slug}/tasks/{id}/queue',
        body: { position: 'end|head', ordinal: 'number?' },
        description: 'Autorizar a Queue con ordinal (Gantt drawer)',
      },
      ...
      whatIf: { method: 'POST', path: '/api/projects/{slug}/gantt/what-if' },
      portfolio: { method: 'GET', path: '/api/gantt/portfolio' },
