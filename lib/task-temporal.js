...
const PATCH_ALIASES = {
  ...
  expectedUnblockDate: 'expected_unblock_date',
...
const LIST_FIELDS = new Set(['assignee', 'required_skills']);
...
function normalizeTaskPatch(input) {
  ...
  for (const [rawKey, rawValue] of Object.entries(updates)) {
    ...
    if (INTEGER_FIELDS.has(canonical)) {
      const bounds = canonical === 'progress'
        ? { min: 0, max: 100 }
        : canonical === 'remaining_ia_hours'
