...
function mejorasBoardPage(project, helpers) {
  ...
  const cards = columns.map(({ status, label, hint, queue }) => {
    ...
    const colItems = sorter(items.filter((task) => taskMatchesColumn(task, status)));
...
.task-list{flex:1 1 auto;display:flex;flex-direction:column;min-height:240px;padding-top:6px}.task{display:block;width:100%;text-align:left;color:#fff;background:#123044;border:1px solid #2f6b8f;border-radius:11px;padding:13px;margin:8px 0}.task[hidden]{display:none!important}.task:hover{border-color:#73d8ff}
...
${QUEUE_BOARD_CSS}
.empty{color:#9db3c7;text-align:center;padding:18px 8px;font-size:12px}
...
.source-editor{width:100%;min-height:320px;background:#08131a;border:1px solid #2f6b8f;border-radius:12px;color:#fff;font:13px/1.5 ui-monospace,Menlo,monospace;padding:14px}
...
<div id="modal" class="modal"><div class="panel"><button class="close">×</button><div class="meta"><span id="detail-substatus" class="substatus"></span></div><h2 id="detail-title"></h2><p id="detail-area" class="muted"></p><div id="detail-view">${boardSubstatusPanelHtml()}<div id="detail-body" class="detail"></div></div><div id="detail-edit" hidden><textarea id="source-editor" class="source-editor"></textarea><div class="edit-actions"><button id="save-task" class="primary">Guardar</button><button id="cancel-edit" class="secondary">Cancelar</button></div></div><p id="action-error" class="muted"></p><div class="actions"><button id="edit-task" class="secondary">Editar texto</button>${boardDeleteButton('Delete')}<button id="copy-action" class="secondary">Copiar instrucción</button></div></div></div>
<script>
...
${boardCreateInitScript({ type: 'enhancement', priority: 'Medium', labels: [], promptText: 'New enhancement title (creates CODE-E-n):', buttonLabel: '+ New enhancement', defaultTemplate: 'enhancement' })}
...
function openTask(task){selectedTask=task;document.querySelector('#detail-title').textContent=(task.id?task.id+' · ':'')+task.title;document.querySelector('#detail-area').textContent='Área: '+task.area;applyTaskDetailHtml(task);document.querySelector('#detail-edit').hidden=true;document.querySelector('#detail-view').hidden=false;actionError.textContent='';if(detailSubstatus){detailSubstatus.textContent=task.effectiveSubstatus||'';detailSubstatus.hidden=!task.effectiveSubstatus}fillSubstatusOptions(task);syncDeleteButton(task);modal.classList.add('open')}
